...
export default function FaqPage(){return <Localized><main className="info-page page-faq">
  ...
  <section className="info-content"><div className="ref-shell info-content-grid"><aside className="info-content-aside"><p className="ref-kicker">WORKING TOGETHER</p><h2>Keep the process clear from the first line.</h2><p>If your question is specific to your project, prepare the brief first, then share it through your preferred contact channel.</p><Link className="ref-btn ghost" href="/start" prefetch>Start Your Line ↗</Link></aside><div className="faq-list">{faqs.map(([question,answer])=><details key={question}><summary>{question}</summary><p>{answer}</p></details>)}</div></div></section>
</main></Localized>}
